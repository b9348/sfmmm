 ...
 * @returns {{ installed: Set<string>, updates: Map<string, string>, loading: boolean }}
 */
export function useInstalledMods() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const rows = await db.select('SELECT mod_key, installed_version FROM installed_workshop_mods')
        if (cancelled) return
  ...
  return { installed, updates, loading }
}
